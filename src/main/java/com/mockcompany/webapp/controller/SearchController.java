    ...
    public Collection<ProductItem> search(@RequestParam("query") String query) {
        ...
        for (ProductItem item : allItems) {
            if (query.contains("'") || query.contains("\"")){
                String unquotedQuery = query.substring(1, query.length() - 1);
                if (item.getName().equals(unquotedQuery) || item.getDescription().equals(unquotedQuery)){
                    itemList.add(item);
                }
            }else{
                String queryL = query.toLowerCase(Locale.ROOT);
                if (item.getName().toLowerCase().contains(queryL) || item.getDescription().toLowerCase().contains(queryL)){
                    itemList.add(item);
                }
            }
        }
//        this.productItemRepository.saveAll(itemList);
        return itemList;
    }
}
